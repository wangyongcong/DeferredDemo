...
static	char*	log(const char *format, ...)
{
	...
	va_list	ap;
	va_start(ap, format);
	length = vsprintf_s(buffer, BUFFER_SIZE, format, ap);
	va_end(ap);

	char* logAddress = nullptr;
	if(length > 0 && length < BUFFER_SIZE - 1)
	{
		buffer[length++] = '\n';
		buffer[length] = 0;
		logAddress = LogToMemory(buffer, unsigned(length));
	}
...
// ---------------------------------------------------------------------------------------------------------------------------------
static void		dumpLine(std::ofstream* fileToWrite, const char* format, ...)
{
	static const uint32_t BUFFER_SIZE = 2048;
	va_list	args;
	...
	if (fileToWrite != NULL)
	{
		(*fileToWrite) << buffer << std::endl;
		fileToWrite->flush();
	}
}

static	void	dumpAllocations(std::ofstream* fh)
{
...
static	void	dumpLeakReport()
{
	{        
		...
		if (stats.totalAllocUnitCount)
		{
			...
			dumpAllocations(&fh);
			dumpLine(&fh, "----All Allocations and Deallocations----\n");
		}
		else
		{
...
void	*mmgrReallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, size_t reportedSize, void *reportedAddress)
{
...

	{
		...
		void	*oldData = malloc(std::min<size_t>(oldReportedSize, reportedSize));
		memcpy(oldData, oldReportedAddress, std::min<size_t>(oldReportedSize, reportedSize));
		...
		{
			// Copy old data
			memcpy(au->reportedAddress, oldData, std::min<size_t>(oldReportedSize, reportedSize));
			au->offset = offset;
		}
...
// ---------------------------------------------------------------------------------------------------------------------------------
static void fsPrintf(std::ofstream* fileStream, const char* format, ...)
{
	static const uint32_t BUFFER_SIZE = 2048;
	va_list	args;
	char buffer[BUFFER_SIZE] = {};
...
#include "nommgr.h"
char* LogToMemory(char* log, size_t logLength=0)
{
	...
	logMemory = (char*)realloc(logMemory, memoryLength + logLength);
	memcpy(logMemory + memoryLength - 1, log, logLength + 1);
